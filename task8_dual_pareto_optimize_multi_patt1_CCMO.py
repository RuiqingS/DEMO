...
def main():
    ...
    for Obj in combinations:
        ...
        for run_idx in range(1, total_runs + 1):
            ...
            Pop2 = evo.Get_Fitness_Pareto(Pop2, dataset_info, device, preds, max_n_nodes, Obj, pattcrops, tolerance, num_workers=num_workers, calCon=False)
            ...
            for gen in range(iters):
                ...
                # 4.1 选择与加噪
                Parent = evo.k_tournament_selection(Pop, 2, int(NPops / 4))
                for i in range(len(Parent)):
                    Parent[i]['AddT'] = current_noise
                    Parent[i]['EvaluatedSC'] = False
                Parent = evo.add_noise(model, Parent, device)

                Parent2 = evo.k_tournament_selection(Pop2, 2, int(NPops / 4))
                for i in range(len(Parent2)):
                    Parent2[i]['AddT'] = current_noise
                    Parent2[i]['EvaluatedSC'] = False
                Parent2 = evo.add_noise(model, Parent2, device)
                ...
                #     sub_off = evo.valOff_Patt(pc_noisy, Parent, min_n_nodes, max_n_nodes, device, max_n_nodes,
                #                               nodes_dist)
                #     Offpatt.extend(sub_off)
                #     sub_off2 = evo.valOff_Patt(pc_noisy, Parent2, min_n_nodes, max_n_nodes, device, max_n_nodes,
                #                               nodes_dist)
                #     Offpatt.extend(sub_off2)

                Off1 = evo.valOff(Parent, min_n_nodes, max_n_nodes, device, max_n_nodes, nodes_dist)
                Off2 = evo.valOff(Parent2, min_n_nodes, max_n_nodes, device, max_n_nodes, nodes_dist)

                lencorss = len(Offpatt + Off1 + Off2)
                Off = evo.denoise_same_level(Offpatt + Off1 + Off2 + Parent + Parent2, model, max_n_nodes, device, dataset_info,
                                             preds,
                                             True)
                Off1 = deepcopy(Off)
                Off2 = deepcopy(Off)

                Pop = evo.Get_Fitness_Pareto(Off1 + Pop, dataset_info, device, preds, max_n_nodes, Obj, pattcrops,
                                              tolerance, num_workers=num_workers)

                Pop2 = evo.Get_Fitness_Pareto(Off2 + Pop2, dataset_info, device, preds, max_n_nodes, Obj, pattcrops,
                                              tolerance, num_workers=num_workers)

                # 4.5 环境选择
                Pop = evo.EnvironmentalSelectionCon(Pop, NPops, Obj)
                Pop2 = evo.EnvironmentalSelectionCon(Pop2, NPops, Obj)

                current_noise = scheduler.update(Off=Off[0:lencorss-1], Pop=Pop, tracker=tracker, viz=viz, obj_names=Obj)
                ...
                print(f"  {MOEA_NAME} {use_EDM} {len(Off)} | Gen {gen+1} | FR: {fr:.2f} | Noise: {current_noise} | HV: {tracker.metrics['HV'][-1]:.4f} | AvgConstraint: {avg_con:.4f} | UVR: {unique_valid_rate:.4f} | UUVR: {UUVR:.4f}")
