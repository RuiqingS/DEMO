    ...
    for Obj in combinations:
        ...
        for NPops in NPops_numbers:
            ...
            for indextime in range(numberoforun):
                ...
                # 初始评价与记录
                Pop = DEMO.EnvironmentalSelection_C_Archive(Pop, NPops, Obj, dataset_info)
                ...
                for iter_idx in range(gens):
                    ...
                    Off = evo.denoise_same_level(Off2 + Parent, model, max_n_nodes, device, dataset_info, preds, True)
                    Off_cross = Off[0: ParentSize-1]

                    # 评价与精英保留
                    Pop = DEMO.EnvironmentalSelection_C_Archive(deepcopy(Pop + Off), NPops, Obj, dataset_info)
                    UUVR = evo.get_population_duplicate_rate(Pop, dataset_info, NPops, usecon=False)
                    current_noise = scheduler.update(Off=Off_cross, Pop=Pop, tracker=tracker, viz=viz, obj_names=Obj)
                    tracker.update(Pop, Obj, AddNoise=current_noise, score=scheduler.score, UUVR=UUVR)
                    print(f"  {MOEA_NAME} {use_EDM} | Gen {iter_idx + 1} |  Noise: {current_noise} | HV: {tracker.metrics['HV'][-1]:.4f}   ")
            ...
            with open(summary_csv_path, 'w', newline='') as f:
                ...
                # 更新表头
                writer.writerow(
                    ['Gen', 'Mean_HV', 'Std_HV', 'Mean_AN', 'Std_AN', 'Mean_Score', 'Std_Score', 'Mean_UUVR', 'Std_UUVR'])
