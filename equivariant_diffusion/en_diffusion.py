...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def log_constants_p_x_given_z0(self, x: object, node_mask: object) -> object:
        """Computes p(x|z0)."""
        batch_size = x.size(0)
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False):
        """
        Draw samples from the generative model with a progress bar.
        """
        # 1. 初始噪声采样
        if fix_noise:
        ...
        # 2. 核心迭代采样 (添加 tqdm 进度条)
        # 包装 reversed(range(0, self.T))
        pbar = tqdm(
            reversed(range(0, self.T)),
            desc="Generative Sampling",
            total=self.T,
            leave=False
        )
        ...
        for s in pbar:
            ...
            z = self.sample_p_zs_given_zt(s_norm, t_norm, z, node_mask, edge_mask, context, fix_noise=fix_noise)
    ...
    @torch.no_grad()
    def repaint(self, n_samples, n_nodes, node_mask, edge_mask, context,
                fixed_x, fixed_h, fixed_mask, n_resamples=1, fix_noise=False):
        """
        Repaint with Correct Subspace Noise Sampling and CoG Balancing.
        """
        # 1. 准备 Fixed 数据 [Batch, N, 3 + Features]
        h = {
            'categorical': fixed_h[:, :, :-1],
            'integer': fixed_h[:, :, -1].long().unsqueeze(-1)
        }
        fixed_x, fixed_h, _ = self.normalize(fixed_x, h, node_mask)
        fixed_h = torch.cat([fixed_h['categorical'], fixed_h['integer']], dim=-1)
        z_fixed_gt = torch.cat([fixed_x, fixed_h], dim=2)

        # 2. 初始化 z_T (Starting Noise)
        # 直接使用全局采样，确保初始状态整体 CoG=0
        if fix_noise:
            z = self.sample_combined_position_feature_noise(1, n_nodes, node_mask)
        else:
            z = self.sample_combined_position_feature_noise(n_samples, n_nodes, node_mask)
        ...
        for s in reversed(range(0, self.T)):
            ...
            # --- Repaint Resampling Loop ---
            for r in range(n_resamples):
                zs_pred = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context,
                                                    fix_noise=fix_noise)
                diffusion_utils.assert_mean_zero_with_mask(zs_pred[:, :, :self.n_dims], node_mask)

                denom = fixed_mask.sum(dim=1, keepdim=True).clamp(min=1e-6)

                pred_fixed_center = torch.sum(
                    zs_pred[:, :, :self.n_dims] * fixed_mask, dim=1, keepdim=True
                ) / denom

                gamma_s = self.gamma(s_array)
                alpha_s = self.alpha(gamma_s, z_fixed_gt)  # 或者用 z_fixed_gt，通常差别不大
                sigma_s = self.sigma(gamma_s, z_fixed_gt)

                fixed_node_mask_bool = (fixed_mask > 0.5).to(fixed_mask.dtype)
                eps_fixed_s = self.sample_combined_position_feature_noise(n_samples, n_nodes,
                                                                          fixed_node_mask_bool)
                zs_fixed = alpha_s * z_fixed_gt + sigma_s * eps_fixed_s
                zs_fixed[:, :, :self.n_dims] = zs_fixed[:, :, :self.n_dims] + pred_fixed_center
                z = fixed_mask * zs_fixed + (1 - fixed_mask) * zs_pred

                diffusion_utils.assert_mean_zero_with_mask(z[:, :, :self.n_dims], node_mask)

                # G. Resampling
                if r < n_resamples - 1 and s > 0:
                    gamma_t = self.gamma(t_array)
                    alpha_t = self.alpha(gamma_t, z_fixed_gt)
                    sigma_t = self.sigma(gamma_t, z_fixed_gt)

                    c1 = alpha_t / (alpha_s + 1e-8)
                    variance = (sigma_t ** 2) - (c1 ** 2) * (sigma_s ** 2)
                    c2 = torch.sqrt(torch.clamp(variance, min=1e-10))

                    eps_t = self.sample_combined_position_feature_noise(n_samples, n_nodes, node_mask)
                    z = c1 * z + c2 * eps_t
                    diffusion_utils.assert_mean_zero_with_mask(z[:, :, :self.n_dims], node_mask)
        ...
        return x, torch.cat([h['categorical'], h['integer']], dim=-1)
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False):
        ...
        """
        z_x, z_h, _ = super().sample(n_samples, n_nodes, node_mask, edge_mask, context, fix_noise)
