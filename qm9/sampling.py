...
def sample(args, device, generative_model, dataset_info,
    ...
    if args.probabilistic_model == 'diffusion':
        x, h, z = generative_model.sample(batch_size, max_n_nodes, node_mask, edge_mask, context, fix_noise=fix_noise)
    ...
    return one_hot, charges, x, node_mask, z

def sample_max_n(max_n_nodes,args, device, generative_model, dataset_info,
           prop_dist=None, nodesxsample=torch.tensor([10]), context=None,
           fix_noise=False):
    #max_n_nodes = dataset_info['max_n_nodes']  # this is the maximum node_size in QM9
...
def sample_sweep_conditional(args, device, generative_model, dataset_info, prop_dist, n_nodes=19, n_frames=100):
    ...
    one_hot, charges, x, node_mask,_ = sample(args, device, generative_model, dataset_info, prop_dist, nodesxsample=nodesxsample, context=context, fix_noise=True)
