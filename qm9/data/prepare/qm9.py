...
from qm9.data.prepare.process import process_xyz_files, process_xyz_gdb9, process_xyz_files_xtb, parse_xtb_molecule
...
def download_dataset_qm9(datadir, dataname, splits=None, calculate_thermo=True, exclude=True, cleanup=True):
    ...
    if splits is None:
        splits = gen_splits_gdb9(gdb9dir, False)

    # Process GDB9 dataset, and return dictionary of splits

    gdb9_data = {}
    if 'xtb' in gdb9dir:
        gdb9_tar_data = join(gdb9dir, 'dsgdb9nsd_xtb.xyz.tar.bz2')
        for split, split_idx in splits.items():
            gdb9_data[split] = process_xyz_files_xtb(
                gdb9_tar_data, parse_xtb_molecule, file_idx_list=split_idx, stack=True)
    else:
        for split, split_idx in splits.items():
