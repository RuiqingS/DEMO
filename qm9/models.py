...
class DistributionNodes:
    ...
    def sample(self, n_samples=1, min_nodes=None, max_nodes=None):
        # 如果未指定范围，直接使用原始分布采样
        ...
        # 构建范围掩码
        mask = torch.ones_like(self.n_nodes, dtype=torch.bool)
        if min_nodes is not None:
            mask = mask & (self.n_nodes >= min_nodes)
        if max_nodes is not None:
            mask = mask & (self.n_nodes <= max_nodes)

        # 筛选范围内的节点及其对应概率
        nodes_in_range = self.n_nodes[mask]
        prob_in_range = self.prob[mask]

        # 检查范围内是否有节点
        if len(nodes_in_range) == 0:
            raise ValueError(f"No nodes in the specified range [{min_nodes}, {max_nodes}]")

        # 重新归一化概率
        prob_in_range = prob_in_range / prob_in_range.sum()

        # 创建临时分布并采样
        temp_dist = Categorical(prob_in_range)
        idx_in_range = temp_dist.sample((n_samples,))
        return nodes_in_range[idx_in_range]
