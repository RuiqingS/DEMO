def main():
    ...
    comb4 = [ ('gap', 'homo'), ('homo', 'lumo'), ('lumo', 'mu'), ('mu', 'Cv'), ('Cv', 'alpha')]
    combinations = comb4 #TODO
    ...
    for Obj in combinations:
        ...
        for run_idx in range(1, total_runs + 1):
            ...
            for gen in range(iters):
                # 4.3 种群内交叉
                Off2 = evo.InitPop(NPops * 50, nodes_dist, args, device, model, dataset_info, prop_dist,
                              False, min_n_nodes, preds, max_n_nodes, True)

                # 4.4 去噪与评估
                lencross = len(Off2)
                Off = evo.denoise_same_level(Off2, model, max_n_nodes, device, dataset_info, preds,True)
                ...
                print(f"  {MOEA_NAME} {use_EDM} {len(Off)} | Gen {gen+1} | FR: {fr:.2f} | Noise: {current_noise} | HV: {tracker.metrics['HV'][-1]:.4f} | AvgConstraint: {avg_con:.4f} | UVR: {unique_valid_rate:.4f} ")
